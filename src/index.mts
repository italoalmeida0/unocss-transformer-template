...
function newContext(
  ...
  if (type === NodeType.CallStatement && !possibleForStatement) result.parent.args = [];
  return result;
}

const clearESC = (text: string) =>
  text.replaceAll('\\\\', ';%PH672%;').replaceAll('\\', '').replaceAll(';%PH672%;', '\\').trim();
...
function errorIfForStatement(errorStore: SyntaxError[], lastRelevantChild: Node | undefined) {
...
function checkLevels(
  ...
  if (
    ...
    ((currentOpen === NodeType.DoubleQuote && !isEscaped) || currentOpen === NodeType.Unknown)
  )
    return { type: NodeType.DoubleQuote };

  if (
    currentChar === SINGLE_QUOTE &&
    ((currentOpen === NodeType.SingleQuote && !isEscaped) || currentOpen === NodeType.Unknown)
  )
...
function parseIsUnknown(
  ...
  if (parentType !== NodeType.CallStatement) {
    if (parentType === NodeType.BlockStatement && currentChar === CLOSE_BLOCK) return 1;
    else if (currentChar === OPEN_BLOCK || currentChar === OPEN_CALL)
      return newContext(
        currentChar === OPEN_BLOCK ? NodeType.BlockStatement : NodeType.CallStatement,
        index,
    ...
    if (checkBracketLevel(currentChar, bracketLevels, OPEN_CALL, CLOSE_CALL)) return 1;
  ...
  while (index < textEnd) {
    ...
    if (parenType === NodeType.CallStatement && possibleForStatement) {
      ...
      if (result === 1) {
        ...
        errorStore.push(syntaxError({ line, column, index, ...(result as SyntaxErrorInput) }));
        endCtx = CLOSE_CALL;
        continue;
      }
    }
    ...
    if (isUnknown) {
      result = parseIsUnknown(index, line, column, ctx, parenType, currentChar, bracketLevels);
      if (result === 1) {
    ...
    if (result === 1) {
      ...
      if (error) {
        errorStore.push(syntaxError({ line, column, index, ...(error as SyntaxErrorInput) }));
    ...
    if (parenType === NodeType.CallStatement) {
      parseCallStatement(currentChar, parent, ctx.wantBracket, childType, token, isEscaped);
    }
    index++;
  }
...
function processTemplateReplacement(template: [string[], string], args: string[]): string {
...
const getContent = (node: Node, text: string) =>
  text.slice((node.start as number) + 1, (node.end as number) - (node.semi ? 2 : 1)).trim();
  ...
  while (index <= end) {
    ...
    if (index >= end) {
      if (standby.length < 1) break;
        ...
        const { cReplacesStore, cIndex, cBody, cScope, cWalkReplaceData, cForLoopData } =
    ...
    if (node.type === NodeType.DirectiveDeclaration && node.id && node.name) {
      ...
      if (type !== -1) {
        ...
        getters.forEach(({ name, filterType, operation, endBlockStart = false }) => {
          if (!filterType(type as DirectiveType) || nodeName !== name) return;
          ...
          if (operation) {
            if (operation[0] === oTemplate) {
              ...
              const resultVariant = processTemplate(scope[variantName]?.[nodeId], args);
              const resultContent = processTemplate(scope[variantContent]?.[nodeId], args);
...
function createCodeFrame(code: string, loc: { line: number; column: number }) {
  ...
  for (let i = startLine; i < endLine; i++) {
    ...
    if (isWithinError) {
      frame += `  ${' '.repeat(maxLineNumberWidth)} | ${' '.repeat(loc.column - 1)}^\n`;
    }
  }
...
function transform(
  ...
  walkDirectiveAST(ctx.ast, text, replacesStore, [rulesSet, rulesGet], weekScope);
  ...
  const r: { code: string; error?: string; ast?: Node } = {
    code:
      sortedReplaces
        .reduce(
          (result, replace) =>
            result.slice(0, replace.start) + replace.content + result.slice(replace.end),
          input
        )
        .trim() + '\n',
  };
...
export default function transformTemplate({
  weekScope = false,
  showErrors = false,
}: Opitions): SourceCodeTransformer {
  return {
    ...
    async transform(code) {
      ...
      const result = transform(original, weekScope);
      if (showErrors && result.error) console.error(result.error);
